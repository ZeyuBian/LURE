...
for (rep in 1:10) {
  ...
  r <- one_rep(dgp, N = 10, TT = 50, epsilon = eps, gamma = gamma)
  res[rep, ] <- r[methods]
}
...
pdf("sim_tabular_plot.pdf", width = 7, height = 5)
par(mar = c(4.5, 4.5, 2.5, 1))
boxplot(res, col = cols, names = methods,
        ylab = "Estimated V(pi)", main = bquote("50 Replications," ~ epsilon == 0.2),
        cex.lab = 1.2, cex.axis = 1.1, outline = FALSE)
abline(h = V_true, lty = 2, lwd = 2, col = "black")
text(5.3, V_true, bquote(V(pi) == .(round(V_true, 2))), pos = 3, cex = 0.9)
dev.off()
cat("\nPlot saved to sim_tabular_plot.pdf\n")
