...
naive_fqe_gym <- function(dat, dgp, gamma, n_iter = 50, ridge = 5.0) {
...
naive_mis_gym <- function(dat, dgp, gamma, ridge = 1e-6) {
  S_mat <- gym_flatten_states(dat$S)
  Sp_mat <- gym_flatten_states(dat$Sp)
  At_vec <- as.vector(dat$Atilde)
  R_vec <- as.vector(dat$R)
...
naive_lstd_gym <- function(dat, dgp, gamma, ridge = 1e-6) {
  S_mat <- gym_flatten_states(dat$S)
  Sp_mat <- gym_flatten_states(dat$Sp)
  At_vec <- as.vector(dat$Atilde)
  R_vec <- as.vector(dat$R)
